...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^lx_") ; then
       LX_BUILD=$(echo -n $1 | sed -e 's/^lx_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $LX_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       LX_BUILD=
    fi
    export LX_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    LX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/lx/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the LX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch lx_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var LX_VERSION)
        ZIPFILE=lx-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.lx.device | grep -q "$LX_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $LX_BUILD, run away!"
    fi
}
...
function cmremote()
{
    ...
    if [ -z "$CMUSER" ]
    then
        git remote add cm ssh://review.cyanogenmod.org:29418/$GERRIT_REMOTE
    else
        git remote add cm ssh://$CMUSER@review.cyanogenmod.org:29418/$GERRIT_REMOTE
    fi
...
function msmremote()
{
    ...
    if [ -z "$MSMUSER" ]
    then
        git remote add msm ssh://review.msm7x30.org:29418/$GERRIT_REMOTE
    else
        git remote add msm ssh://$MSMUSER@review.msm7x30.org:29418/$GERRIT_REMOTE
    fi
    echo "Remote 'msm' created"
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.lx.device | grep -q "$LX_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
