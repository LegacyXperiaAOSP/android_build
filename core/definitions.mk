...
$(eval PREBUILT_CACHE_HIT := $(shell build/tools/shlib-cache-check \
		"$(MODULE_CACHE_PATH)" \
		"$(LOCAL_PATH)" \
		"LOCAL_CC=$(LOCAL_CC)" \
		"LOCAL_CXX=$(LOCAL_CXX)" \
...
$(MODULE_CACHE_PATH)/lib.so: $(__BUILT_MODULE)
	@mkdir -p $$(@D)
	$(hide) cp $$^ $$@
	$(hide) build/tools/shlib-cache-enter \
...
$(MODULE_CACHE_PATH)/lib.so: $(__BUILT_MODULE)
	...
	$$(hide) build/tools/shlib-cache-enter \
		...
		"LOCAL_SRC_FILES=$$(PRIVATE_SRC_FILES)"
$(__INSTALLED_MODULE): $(MODULE_CACHE_PATH)/symbols.so
$(MODULE_CACHE_PATH)/symbols.so: $(__LINKED_MODULE)
	@mkdir -p $$(@D)
	$$(hide) cp $$^ $$@
endef
...
$(__SYMBOLS_MODULE): $(MODULE_CACHE_PATH)/symbols.so
	@mkdir -p $$(@D)
	$$(hide) cp $$^ $$@
endef
